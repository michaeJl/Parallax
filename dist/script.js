...
var scroll = function scroll() {
  var layers = document.querySelectorAll(".img_parallax");
  layers.forEach(function (item) {
    var lays = getCoords(item);
    var translate3d = "translate3d(-50%," + -100 + "px, 0px)";
    item.style.transform = translate3d;
  });
  window.addEventListener('scroll', function () {
    var topDistance = pageYOffset;
    var movement = -(topDistance * 0.2);

    for (var layer in layers) {
      var lays = getCoords(layers[layer]);
      ...
      if (lays) {
        if (topDistance + document.documentElement.clientHeight * 2 >= lays.top && topDistance - document.documentElement.clientHeight * 2 <= lays.bottom) {
          var translate3d = "translate3d(-50%," + (-100 + movement) + "px, 0px)";
